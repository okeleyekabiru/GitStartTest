...
namespace Product.Tests
{
    public class ProductServiceTests
    {
        ...
        {
            _mockProductRepo = new Mock<IGenericRepository<Product.API.Domain.Entities.Product>>();
        ...
        {
            var products = Task.FromResult(new List<Product.API.Domain.Entities.Product>
            {
                new Product.API.Domain.Entities.Product { Id = Guid.NewGuid(), Name = "Product 1", Price = 10.00m, StockQuantity = 5 },
                new Product.API.Domain.Entities. Product { Id = Guid.NewGuid(), Name = "Product 2", Price = 20.00m, StockQuantity = 15 }
            }.AsQueryable());
        ...
        public async Task CreateProductAsync_CreatesProduct()
        {
            ...
            _mockProductRepo.Setup(repo => repo.AddAsync(It.IsAny<Product.API.Domain.Entities.Product>())).Returns(Task.CompletedTask);
        ...
        {
            var productId = Guid.NewGuid();
            var existingProduct = new Product.API.Domain.Entities.Product { Id = productId, Name = "Old Product", Price = 20.00m, StockQuantity = 5 };
            ...
            _mockProductRepo.Setup(repo => repo.UpdateAsync(It.IsAny<Product.API.Domain.Entities.Product>())).Returns(Task.CompletedTask);
